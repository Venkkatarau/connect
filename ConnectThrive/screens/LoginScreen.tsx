...
import { setGlobalUser } from '../config/globalUser';
import {
  View,
  Text,
  TextInput,
...
const LoginScreen = () => {
  ...
  const fetchAndSetUser = async (name: string, phone: string) => {
    try {
      const res = await fetch(`${BASE_URL}/v1/user/by-mobile?mobileNumber=${phone}`);
      if (res.ok) {
        ...
        setGlobalUser(
          ...
          userDetails.id,
          userDetails.batchId || 1
        );
      } else {
        setGlobalUser(name, phone);
      }
    } catch (error) {
      console.error("Failed to fetch dynamic user info:", error);
      setGlobalUser(name, phone);
    }
  };
  ...
  const handleSignupWithOtp = async (enteredOtp: string[]) => {
    try {
      ...
      if (response.ok && data.status == false) {
        // "Mobile number already registered" means user exists — navigate to home
        if (data.message?.includes("already registered")) {
          await fetchAndSetUser(username, mobileNumber);
    ...
    if (isFormValid) {
      ...
      try {
        const response = await fetch(
          `${BASE_URL}/v1/request-signup-otp?mobileNumber=${mobileNumber}`,
          {
            method: "POST",
            headers: {
  ...
  const isFormValid =
    username.trim().length > 0 && /^[0-9]{10}$/.test(mobileNumber);
            ...
            {timer > 0 ? (
              ...
              <Text style={styles.resendText}>
                ...
                <Text
                  ...
                  onPress={handleSubmit} // 👈 calls your OTP request API again
                >
                  Resend Now
                </Text>
              </Text>
...
const styles = StyleSheet.create({
  ...
  resendLink: {
    color: "#225663", // or purple like zingbus
    fontWeight: "600",
  },
});
